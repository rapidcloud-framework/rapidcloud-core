...
args = getResolvedOptions(sys.argv, ["job_name", "env", "dataset"])
job_name = args["job_name"].lower()
env = args["env"].lower()
table_name = args["dataset"].lower()
env_hyphens = env.replace("_", "-")
...
s3_kinesis_checkpoint = f"s3://{env_hyphens}-raw/kinesis_event_checkpoint/"
s3_kinesis_event = f"s3:///{env_hyphens}-raw/kinesis_emr_event"
database = f"{env}_ingestiondb"
    ...
    if data_frame.count() > 0:
        ...
        apply_mapping = ApplyMapping.apply(
            ...
            mappings=[
                ...
                ("registration_ts", "string", "registration_ts", "string"),
                ("start_ts", "string", "start_ts", "string"),
                ("status", "string", "status", "string"),
                ("swagger_doc", "string", "swagger_doc", "string"),
                ("type", "string", "type", "string"),
                ("version", "string", "version", "string"),
                ("domain_id", "int", "domain_id", "int"),
                ("source_database", "string", "source_database", "string"),
                ("source_schema", "string", "source_schema", "string"),
...
glueContext.forEachBatch(
    ...
    options={"windowSize": "100 seconds", "checkpointLocation": s3_kinesis_checkpoint},
)
