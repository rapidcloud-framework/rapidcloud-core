...
def put_item(table, item):
    ...
    logger.info(json.dumps(response, indent=2, default=json_converter))
...
try:
    ...
    logger.info(f"transform_name: {transform_name}")

    # transform_log
    transform_fqn = start_transform(env, dataset, job_name, args)['fqn']

    # transform metadata
    transform = query("transform", "fqn", transform_name)    
    logger.info(json.dumps(transform, indent=2, default=json_converter))
