...
class CloudMetadata(object):
    ...
    def load_cloud_config(self):
        self.logger.debug(f"ng config/{self.cloud}_profile.json")
        ...
        with open(f'config/{self.cloud}_profile.json') as f:
            ...
            if self.cloud == "aws":
                ...
                elif hasattr(self.args, "aws_profile") and self.args.aws_profile:
                    self.aws_profile = self.args.aws_profile
                else:
                    self.aws_profile = self.config.get('aws_profile')
                self.logger.info(f"load_cloud_config: profile:{self.aws_profile}, region:{self.region_name}, role:{self.x_acct_role_arn}")
