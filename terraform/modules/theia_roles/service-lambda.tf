...
resource "aws_iam_role" "service_lambda_role" {
  name = "${var.prefix}-service-lambda"
...
resource "aws_iam_role_policy_attachment" "service_lambda_attach" {
  role       = aws_iam_role.service_lambda_role.name
  policy_arn = aws_iam_policy.service_lambda_policy.arn
}

data "aws_iam_policy_document" "service_lambda_policy_json" {
...
resource "aws_iam_policy" "service_lambda_policy" {
  name   = "${var.prefix}-service-lambda"
  policy = data.aws_iam_policy_document.service_lambda_policy_json.json
}

resource "aws_iam_role_policy_attachment" "service_attach_lambda_generic" {
  role       = aws_iam_role.service_lambda_role.name
  policy_arn = aws_iam_policy.lambda_generic_policy.arn
}

output "service_lambda_role_arn" {
  value = aws_iam_role.service_lambda_role.arn
}
