resource "aws_iam_role" "ingestion_raw_glue_role" {
  name = "${var.prefix}-ingestion-raw-glue"
...
resource "aws_iam_role_policy_attachment" "ingestion_raw_glue_attach" {
  role       = aws_iam_role.ingestion_raw_glue_role.name
  policy_arn = aws_iam_policy.ingestion_raw_glue_policy.arn
}

data "aws_iam_policy_document" "ingestion_raw_glue_policy_json" {
...
resource "aws_iam_policy" "ingestion_raw_glue_policy" {
  name   = "${var.prefix}-ingestion-raw-glue"
  policy = data.aws_iam_policy_document.ingestion_raw_glue_policy_json.json
}

resource "aws_iam_role_policy_attachment" "ingestion_raw_attach_glue_service_role" {
  role       = aws_iam_role.ingestion_raw_glue_role.name
  policy_arn = "${var.arn_prefix[var.region]}:iam::aws:policy/service-role/AWSGlueServiceRole"
}

output "ingestion-raw_glue_role_arn" {
  value = aws_iam_role.ingestion_raw_glue_role.arn
}
