...
def get_existing_tag_set(bucket_name, object_key):
    try:
        ...
        logger.error('failed to get existing tags: ' + str(ex))
        return None
...
def get_metadata(bucket_name, object_key):
    try:
        ...
        logger.error('failed to get existing metadata: ' + str(ex))
        return None
...
def copy_object(source_bucket, source_key, dest_bucket, dest_key, tags, metadata, acl=None):
    ...
    logger.info(json.dumps(copy_source, indent=2, default=str))
...
def lambda_handler(event, context):
    ...
    quarantine_bucket = f"{env}-quarantine".replace('_','-')
    logger.info(f"quarantine_bucket: {quarantine_bucket}")
    ...
    for record in event['Records']:
        ...
        if message['scanner_status'] != 0:
            logger.info('Skip: ', message['scanner_status_message'])
            continue
        ...
        if promote_bucket is None:
            env_partition = object_key.split("/")[0]
            dest_env = env_partition.replace("env=","")
            promote_bucket = f"{dest_env}-ingestion".replace('_','-')
            dest_object_key = object_key.replace(env_partition + '/', '')
        logger.info(f"promote_bucket: {promote_bucket}")
        logger.info(f"dest_object_key: {dest_object_key}")
        ...
        if not dst_bucket:
            logger.info(f'Skip: No bucket specified for {operation}')
            continue
        ...
        logger.info(f'File {operation} successful (mode: {mode})')